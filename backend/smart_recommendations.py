"""
==========================================================
Aoun (عون) — Smart Recommendations Engine
==========================================================

Replaces the flat RULES dict in backend_main.py with an engine
that personalizes tips based on:

  • Specific symptom VALUES (not just risk level)
      e.g. fatigue=9 → nap & pacing tips
           nausea=8 → ginger + small meals tips
           shortness_of_breath=7 → breathing exercises

  • Lifestyle LOGS from the last 7 days
      e.g. water_intake < 1.5 L avg → hydration nudge
           sleep_hours < 6 avg → sleep hygiene
           exercise_mins < 10 avg → gentle movement
           diet_quality < 4 avg → meal suggestions

  • Cancer TYPE (lung/breast/colon/etc.)
      Each gets disease-specific guidance

  • Treatment DURATION
      Early (<30d) / mid / late patients need different advice

  • Logging CONSISTENCY
      Encouragement if patient hasn't logged in 3+ days

Returns 6–10 tips, ranked by urgency/relevance.
==========================================================
"""
