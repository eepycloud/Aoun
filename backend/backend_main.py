...
async def log_symptoms(patient_id: int, payload: schemas.SymptomInput, db: Session = Depends(get_db)):
    ...
    if invalid_fields:
        raise HTTPException(422, f"Values out of range (0-10): {', '.join(invalid_fields)}")
    ...
    if prediction and prediction.get("risk_level") == "High":
        # ✅ Patient sees: contact oncologist message
        db.add(models.Alert(
            patient_id=patient_id, symptom_id=record.id,
            alert_type="HighRisk", risk_level="High",
            message="High risk detected. Please contact your oncologist.",
            sent_to_patient=True, sent_to_doctor=False,
            doctor_id=p.confirmed_by_doctor,
        ))
        # ✅ Doctor sees: review patient urgently message
        db.add(models.Alert(
            patient_id=patient_id, symptom_id=record.id,
            alert_type="HighRisk", risk_level="High",
            message=f"Patient {p.full_name} has been flagged as High risk. Please review urgently.",
            sent_to_patient=False, sent_to_doctor=True,
            doctor_id=p.confirmed_by_doctor,
        ))
        db.commit()
        alert_created = True
...
def get_alerts(patient_id: int, db: Session = Depends(get_db)):
    alerts = (
        ...
        .filter(
            ...
            models.Alert.sent_to_patient == True  # ✅ Only show patient alerts
        )
...
def get_doctor_alerts(doctor_id: int, db: Session = Depends(get_db)):
    alerts = (
        db.query(models.Alert)
        .filter(models.Alert.doctor_id == doctor_id,
                models.Alert.acknowledged == False,
                models.Alert.sent_to_doctor == True)  # ✅ Only doctor alerts
...
async def save_wellness_scan(
    ...
    if payload.severity in ("medium", "high"):
        ...
        patient_message = (
            ...
            f"Please inform your doctor if you feel unwell."
        )
        # ✅ Doctor message
        doctor_message = (
            f"Patient {p.full_name} face wellness scan: {payload.prediction}. "
            f"Illness score: {payload.illness_score}/100. "
            f"Pallor: {payload.pallor_score}, Eye fatigue: {payload.eye_fatigue}, "
            f"Skin dullness: {payload.skin_dullness}. Please review."
        )
        ...
        if payload.severity == "high" and p.confirmed_by_doctor:
            await _send_firebase(
                ...
                body=f"Patient {p.full_name} appears unwell. Score: {payload.illness_score}/100.",
            )
