"""
aoun_rag.py v3 - Semantic RAG engine with feedback-weighted retrieval.

v3 changes:
  - Retrieval returns source_id for every hit (ChromaDB document id)
  - New function: apply_feedback_weights() adjusts the ranking of a
    set of results using a feedback map {source_id: net_rating}
  - Negative net ratings penalize; positive net ratings boost
  - Sources with rating <= BLOCK_THRESHOLD get filtered out entirely
"""
...
# ---- RAG Query Cache ----
# Caches embedding + retrieval results so repeated questions skip
# the expensive encode+ChromaDB round-trip entirely.
# Cache is keyed by (normalized_query, top_k) → list of hits.
# TTL: entries expire after RAG_CACHE_TTL_SECONDS (default 10 min).

import time as _time
import hashlib as _hashlib

RAG_CACHE_TTL_SECONDS = int(os.getenv("RAG_CACHE_TTL", "600"))  # 10 min default
_rag_cache: dict = {}  # key -> {"hits": [...], "expires": float}


def _rag_cache_key(query: str, top_k: int) -> str:
    normalized = " ".join(query.lower().split())
    return _hashlib.md5(f"{normalized}|{top_k}".encode()).hexdigest()


def _rag_cache_get(query: str, top_k: int):
    key = _rag_cache_key(query, top_k)
...
def _rag_cache_set(query: str, top_k: int, hits: list):
    key = _rag_cache_key(query, top_k)
    _rag_cache[key] = {
        "hits": hits,
        "expires": _time.time() + RAG_CACHE_TTL_SECONDS,
    }
    print(f"[RAG CACHE] Stored {len(hits)} hits for: {query[:50]}")


def clear_rag_cache():
    """Clear the RAG cache — call this after ingesting new PDFs."""
    count = len(_rag_cache)
    _rag_cache.clear()
    print(f"[RAG CACHE] Cleared {count} entries")
    return count
...
def search_knowledge(query: str, top_k: int = 3,
    ...
    init()
    try:
        # ── RAG cache fast-path (skip embed + ChromaDB when no feedback rerank) ──
        if not feedback_map:
        ...
        # Store in RAG cache for next time (only when no feedback rerank)
        if not feedback_map:
            _rag_cache_set(query, top_k, final)
